...
export const LotrLCGInfo = () => {
  const { playerId, moves, undo, redo, ctx, leave } = useContext(StateContext);
  const state = ctx.state;

  const totalWillpower = sum(
    values(ctx.state.cards)
      .filter((c) => c.mark.questing)
      .map((c) => ctx.cards[c.id].props.willpower ?? 0)
  );

  const totalThreat = ctx.getNumber('totalThreat');
  ...
  const targetProgress = sum(
    state.zones.questArea.cards.map((id) => ctx.cards[id].props.questPoints)
  );
          ...
          <Stack direction="row" style={{ pointerEvents: 'auto' }}>
            ...
            <IconButton
              onClick={() => {
                ...
                console.log('view', ctx.modifiers);
              }}
            >
        ...
        {state.modifiers.length > 0 && (
          ...
          >
            ...
            {state.modifiers.map((m, i) => {
              const title = ctx.cards[m.source].props.name ?? '';
              return (
                <Fragment key={title + i}>
                  <Tooltip title={title} placement="left">
                    <Typography>{JSON.stringify(m)}</Typography>
                  </Tooltip>
